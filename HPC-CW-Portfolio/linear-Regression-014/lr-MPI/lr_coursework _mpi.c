...
int main() {
  ...
  MPI_Comm_rank(MPI_COMM_WORLD,&rank);
   ...
   if (size !=9){
  printf("This program needs exactly 9 instances.\n");
  exit(-1);
  ...
  while(!minimum) {
       ...
       if (rank != 0)
      {
         ...
         dm[i] = bm + (om[i] * step);
         dc[i] = bc + (oc[i] * step);
         dataError = rms_error (dm[i], dc[i]);

         MPI_Send (&dataError, 1, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD);
         ...
         MPI_Recv (&minimum, 1, MPI_INT, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
      }
      else
      {
         for (i = 1; i < size; i++)
         {
            MPI_Recv (&dataError, 1, MPI_DOUBLE, i, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            ...
            if (dataError < best_error)
            {
               best_error = dataError;
               best_error_i = i - 1;
         ...
         for (i = 1; i < size; i++)
         {
            ...
            MPI_Send (&minimum, 1, MPI_INT, i, 0, MPI_COMM_WORLD);
         }
      }
   }
