...
__device__ double device_residual_error(double x, double y, double m, double c) {
  double e = (m * x) + c - y;
  return e * e;
}
__global__ void device_rms_error(double *m, double *c, double *sum_error, point_t *data_d) {
	
	int i = threadIdx.x + blockIdx.x * blockDim.x;

  sum_error[i] = device_residual_error(data_d[i].x, data_d[i].y, *m, *c);
}
...
int main() {
  ...
  double *data_dm;
  double *data_dc;
  double *data_sum_error;
  point_t *data_d;

        //Allocate memory for data_dm
	error = cudaMalloc(&data_dm, (sizeof(double) * 8));
 	if(error){
   	fprintf(stderr, "cudaMalloc on data_dm returned %d %s\n", error,
    	cudaGetErrorString(error));
   	exit(1);
 	}
	
	//Allocate memory for data_dc
	error = cudaMalloc(&data_dc, (sizeof(double) * 8));
 	if(error){
   	fprintf(stderr, "cudaMalloc on data_dc returned %d %s\n", error,
  	  cudaGetErrorString(error));
   	exit(1);
 	}
	
	//Allocate memory for data_sum_error
	error = cudaMalloc(&data_sum_error, (sizeof(double) * 1000));
 	if(error){
   	fprintf(stderr, "cudaMalloc on data_sum_error returned %d %s\n", error,
   	  cudaGetErrorString(error));
   	exit(1);
 	}

	//Allocate memory for data_d
	error = cudaMalloc(&data_d, sizeof(data));
 	if(error){
   	fprintf(stderr, "cudaMalloc on data_d returned %d %s\n", error,
   	  cudaGetErrorString(error));
   	exit(1);
 	}
  	...
  	while(!minimum_found) {
		...
		//Copy memory for dm to data_dm
  	error = cudaMemcpy(data_dm, dm, (sizeof(double) * 8), cudaMemcpyHostToDevice);  
  	if(error){
    	fprintf(stderr, "cudaMemcpy to data_dm returned %d %s\n", error,
      cudaGetErrorString(error));
  	}

		//Copy memory for dc to data_dc
  	error = cudaMemcpy(data_dc, dc, (sizeof(double) * 8), cudaMemcpyHostToDevice);  
  	if(error){
    	fprintf(stderr, "cudaMemcpy to data_dc returned %d %s\n", error,
      cudaGetErrorString(error));
  	}

		//Copy memory for data to data_d
  	error = cudaMemcpy(data_d, data, sizeof(data), cudaMemcpyHostToDevice);  
  	if(error){
    	fprintf(stderr, "cudaMemcpy to data_d returned %d %s\n", error,
      cudaGetErrorString(error));
  	}
		
    		for(i=0;i<8;i++) {

			double Host_sum_error[1000];
			double total_error_sum;
			double mean_sum_error;
			device_rms_error <<<10,100>>>(&data_dm[i], &data_dc[i], data_sum_error, data_d);
			cudaThreadSynchronize();

			//Copy memory for data_sum_error
		  error = cudaMemcpy(&Host_sum_error, data_sum_error, (sizeof(double) * 1000),  					cudaMemcpyDeviceToHost);  
		  if(error){
	    fprintf(stderr, "cudaMemcpy to error_sum returned %d %s\n", error,
	      cudaGetErrorString(error));
		  }

			//Loop for the error_sum returned from the kernel function
			for( l=0; l<n_data; l++) {
				//Add each error sum to the error sum total.
    		total_error_sum += Host_sum_error[l];
  		}

			//Calculate the mean for the error sum.
			mean_sum_error = total_error_sum / n_data;

			//Calculate the square root for the error sum mean.
			e[i] = sqrt(mean_sum_error);
	...
	//Free memory for data_dm
	error = cudaFree(data_dm);
	if(error){
		fprintf(stderr, "cudaFree on data_dm returned %d %s\n", error,
	  	cudaGetErrorString(error));
		exit(1);
	}
	
	//Free memory for data_dc
	error = cudaFree(data_dc);
	if(error){
		fprintf(stderr, "cudaFree on data_dc returned %d %s\n", error,
			cudaGetErrorString(error));
		exit(1);
	}

	//Free memory for data_d
	error = cudaFree(data_d);
	if(error){
		fprintf(stderr, "cudaFree on data_d returned %d %s\n", error,
	  	cudaGetErrorString(error));
	 	exit(1);
	}
		
	//Free memory for data_sum_error
	error = cudaFree(data_sum_error);
	if(error){
		fprintf(stderr, "cudaFree on data_sum_error returned %d %s\n", error,
	  	cudaGetErrorString(error));
	 	exit(1);
	}
